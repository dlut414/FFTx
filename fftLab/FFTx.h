...
public:
	...
	__forceinline C W(const unsigned& k) const {
		...
		const T theta = static_cast<T>(-2 * PI* k / N);
		...
		for (unsigned step = 1; step < (nn); step <<= 2) {
			...
			for (unsigned factorId = 0; factorId < step; factorId++) {
				...
				for (unsigned a = factorId; a<(nn); a += jump) {
					...
					C res1 = data[a] + C(0, -1)*factor1*data[b] + C(-1, 0)*	factor2*data[c] + C(0, 1)*	factor3*data[d];
					C res2 = data[a] + C(-1, 0)*factor1*data[b] + C(1, 0)*	factor2*data[c] + C(-1, 0)*	factor3*data[d];
					C res3 = data[a] + C(0, 1)*	factor1*data[b] + C(-1, 0)*	factor2*data[c] + C(0, -1)*	factor3*data[d];
					data[a] = res0;
					data[b] = res1;
					data[c] = res2;
					data[d] = res3;
		...
		for (unsigned step = 1, groupSize = nn; step < (nn); step <<= 2, groupSize >>= 2) {
			for (unsigned factorId = 0; factorId < step; factorId++) {
				...
				for (unsigned i = 0; i < groupSize; i += 4) {
					...
					shift[bb] = data[a] + C(0, -1)*factor1*data[b] + C(-1, 0)*	factor2*data[c] + C(0, 1)*	factor3*data[d];
					shift[cc] = data[a] + C(-1, 0)*factor1*data[b] + C(1, 0)*	factor2*data[c] + C(-1, 0)*	factor3*data[d];
					shift[dd] = data[a] + C(0, 1)*	factor1*data[b] + C(-1, 0)*	factor2*data[c] + C(0, -1)*	factor3*data[d];
				}
			}
			for (unsigned i = 0; i < nn; i++) {
				data[i] = shift[i];
